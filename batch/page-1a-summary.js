(function () {
  ...
  // 3학년 1학기인 경우: 병원, 부서, 내과/외과 (3개 열)
  // 4학년 1학기인 경우: 과목, 분기(선택사항), 부서(선택사항), 병원 (4개 열)
  // 일반 경우: 과목, 병원 (2개 열)
  const infoCols = isGrade3Semester1 ? 3 : (isGrade4Semester1 ? 4 : 2);
  ...
  if (isGrade3Semester1) {
    ...
    headRow.appendChild(th('부서'));
    headRow.appendChild(th('내과/외과'));
  } else if (isGrade4Semester1) {
    headRow.appendChild(th('과목'));
    headRow.appendChild(th('분기'));
    headRow.appendChild(th('병원'));
    headRow.appendChild(th('부서'));
  } else {
    headRow.appendChild(th('과목'));
  ...
  for (let r = 0; r < rowsLen; r++) {
    ...
    if (isGrade3Semester1) {
      ...
      tr.appendChild(td(maxRow.department || minRow.department || ''));
      const deptType = maxRow.deptType || minRow.deptType || '';
      let deptTypeText = '';
      if (deptType === 'internal') {
        deptTypeText = '내과';
      } else if (deptType === 'external') {
        deptTypeText = '외과';
      } else if (deptType === 'both') {
        deptTypeText = '내과&외과';
      }
      tr.appendChild(td(deptTypeText));
    } else if (isGrade4Semester1) {
      tr.appendChild(td(maxRow.subject || minRow.subject || ''));
      tr.appendChild(td(maxRow.branch || minRow.branch || '')); // 분기는 선택사항
      tr.appendChild(td(maxRow.hospital || minRow.hospital || ''));
      tr.appendChild(td(maxRow.department || minRow.department || '')); // 부서는 선택사항
    } else {
      tr.appendChild(td(maxRow.subject || minRow.subject || ''));
      tr.appendChild(td(maxRow.hospital || minRow.hospital || ''));
    }
    ...
    // 최소 인원이 없으면 최대 인원만 표시
    const totalText = rowTotalMin > 0 ? `${rowTotalMax} / ${rowTotalMin}` : String(rowTotalMax);
    tr.appendChild(td(totalText));
    ...
    for (let p = 1; p <= numPeriods; p++) {
      ...
      // 최소 인원이 없으면 최대 인원만 표시
      const text = minVal != null ? `${maxVal ?? ''} / ${minVal ?? ''}` : (maxVal ?? '');
  ...
  if (isGrade3Semester1) {
    // 3학년 1학기: 병원, 부서, 내과/외과 (3개 열)
    totalRow.appendChild(td(''));
    totalRow.appendChild(td(''));
  } else if (isGrade4Semester1) {
    // 4학년 1학기: 과목, 분기, 병원, 부서 (4개 열)
    totalRow.appendChild(td(''));
    totalRow.appendChild(td(''));
    totalRow.appendChild(td(''));
  } else {
  ...
  // 최소 인원이 없으면 최대 인원만 표시
  const grandTotalText = grandTotalMin > 0 ? `${grandTotalMax} / ${grandTotalMin}` : String(grandTotalMax);
  totalRow.appendChild(td(grandTotalText));
  for (let p = 0; p < numPeriods; p++) {
    const periodText = periodTotalsMin[p] > 0 ? `${periodTotalsMax[p]} / ${periodTotalsMin[p]}` : String(periodTotalsMax[p]);
    totalRow.appendChild(td(periodText));
  }
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      ...
      const payload = {
        ...
        selectedSemester: selectedSemesterInfo || null
      };
